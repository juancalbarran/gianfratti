...
this.width =  "100%";
this.height = "200";
this.bodyStyle = 'background-color: white; font-family: "Arial"; font-size: x-small; border-style: ridge; border-color: #D8D8D8';
this.bordersStyle = 'border:1px dotted #ff0000;height:20px';
this.imgURL = _editor_url + '../Bibliotecas/htmlarea/images/';
this.debug  = 0;
...
//Persistent Buttons
	['about','separator','htmlmode','popupeditor','separator',
	'find','separator','changecase','separator'],['ltr','rtl','separator'],['linebreak'],['refresh'],
	//	['help'],
...
//File
	...
	//Edit	
	['selectall','cut','copy','paste','delete','remove','undo','redo','separator',
//Format Text Style
...
//Format Text Direction
	'separator','cleancode','separator',],
	['linebreak'],
//Row 3: Insert/Modify Tools	
	['paragraph','marquee','line','specchar','separator',
	//Hyperlinks and Images
	'createlink','unlink','anchor','InsertImage','separator',
	//Tables
	'InsertTable','showborder','TableProperties','separator',
	//Table Rows
	'RowProperties','InsertRowBefore','InsertRowAfter','DeleteRow','SplitRow','MergeRows','separator',
	//Table Columns
	'InsertColumnBefore','InsertColumnAfter','DeleteColumn','separator',
	//Table Cells
...
//Row 2: File, Edit, Format Tools
//File
"refresh":         	['Refresh',             'Clean',                'editor_action(this.id)',   'ed_refresh.gif'],
...
"redo":            	['Redo',                'Redo',               	'editor_action(this.id)',  'ed_redo.gif'],
...
"justifynone":     	['JustifyNone',		'Remove Justify',	'editor_action(this.id)',  'ed_align_none.gif'], 
//Format Text Block
...
"insertimage2":		['InsertImage2',	'Insert Image',		'editor_action(this.id)',  'ed_image.gif'],
...
"insertcolumnafter":  	['InsertColumnAfter',   'Insert Column After',	'editor_action(this.id)',  'ed_insright.gif'],
...
// end insert by lvn
      ...
      // fontsize
      if (btnName == "fontsize") {
        toolbar += '  Font Size <select id="_' +objname+ '_FontSize" onchange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 12px;	scrollbar-face-color : Fuchsia;">';
...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(_editor_url + "../Bibliotecas/htmlarea/popups/editor_help.html", 'EditorHelp',
  ...
  if (cmdID == 'popupeditor') {
    window.open(_editor_url + "../Bibliotecas/htmlarea/popups/fullscreen.html?"+objname,
                'FullScreen',
                'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes');
    return;
  }
...
// If you don't wan't the Find-function to work in HTML-mode, see http://www.interactivetools.com/forum/forum.cgi?post=12934
  if (cmdID == 'Find') {
     ...
     showModelessDialog(_editor_url + "../Bibliotecas/htmlarea/popups/find.html",window, "resizable: no; help: no; status: no; scroll: no; ");
     return;
  }
// end insert by lvn  
...
// inserted by lvn
  ...
  // special characters
    ...
    var newchar = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_char.html", '', "dialogWidth:493px; dialogHeight: 140px; resizable: no; help: no; status: no; scroll: no;");
    if (newchar == '') {
       return;
    } else {
...
// end insert by lvn
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
	...
	var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_easylink.html", editdoc, "resizable: no; help:no; status: no; scroll: no; ");
	if (myText) {
...
showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_image.html", editdoc, "resizable: no; help:no; status: no; scroll: no; ");
    }

// insert image 2 (For Fullxml regular users)
else if (cmdID.toLowerCase() == 'insertimage2'){
showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_image2.html", editdoc, "resizable: no; help:no; status: no; scroll: no; ");
    }
	

// insert form
  else if (cmdID.toLowerCase() == 'inputform') {
	var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_form.html","","resizable: no; help: no; status: no; scroll: no; ");
	if (myText) {
		editor_insertHTML(objname, unescape( myText) );
	}
  }
...
// insert formelement 
		...
		var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_formelement.html",
									highlightedText,
    ...
    // insert table
      ...
      setGlobalVar('_editor_field',objname);
      showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_table.html?"+objname,
                                 window,
...
// insert line
    ...
    setGlobalVar('_editor_field',objname);
    var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_line.html",
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
...
// Change Case
	...
	var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/changecase.html",
...
// insert marquee
   ...
   var myTitle = "";
    var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_marquee.html",
...
// Insert anchor
   ...
   var myTitle = "";
    var myText = showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/insert_anchor.html",
                                 myTitle,      // str or obj specified here can be read from dialog as "window.dialogArguments"
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    if (myText) { editor_insertHTML(objname, unescape( myText) ); }
  }
...
// inserted by lvn : property pallettes
           case 'TableProperties'    : nullBorders(editor_obj.contentWindow.document,'hide'); 
                                       ...
                                       showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/tableprop.html?"+objname,
                                                       window,
                                                       "resizable: yes; help: no; status: no; scroll: no; ");
                                       td.focus();
                                       break;
           case 'RowProperties'      : setGlobalVar('_editor_field',objname);
                                       ...
                                       showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/rowprop.html?"+objname,
                                                       window,
                                                       "resizable: yes; help: no; status: no; scroll: no; ");
                                       td.focus();
                                       break;
           case 'CellProperties'     : setGlobalVar('_editor_field',objname);
                                       ...
                                       showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/cellprop.html?"+objname,
                                                       window,
                                                       "resizable: yes; help: no; status: no; scroll: no; ");
                                       td.focus();
                                       break;
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ '../Bibliotecas/htmlarea/popups/blank.html"
...
// set event handlers
  ...
  //
    ...
    html += '</style>\n'
      + '</head>\n'
      + '<body contenteditable="true" topmargin=1 leftmargin=1 dir=ltr'
...
function editor_about(objname) {
  showModalDialog(_editor_url + "../Bibliotecas/htmlarea/popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}
