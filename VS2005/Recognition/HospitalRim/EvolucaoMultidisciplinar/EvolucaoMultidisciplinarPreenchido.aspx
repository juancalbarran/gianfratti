<%@ Page Language="C#" MasterPageFile="~/Template/TemplateEmpresa.master" AutoEventWireup="true" CodeFile="EvolucaoMultidisciplinarPreenchido.aspx.cs" Inherits="EvolucaoMultidisciplinar_EvolucaoMultidisciplinarPreenchidot" Title="Untitled Page" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
	<fieldset>
		<legend align="center">Evolução Multidisciplinar</legend>
<fieldset>
        <legend>Dados Paciente</legend>
  <table class="texto" width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="30%">Nome do Paciente </td>
            <td width="25%">Registro Geral </td>
            <td width="15%">Quarto/Leito</td>
            <td width="25%">Data</td>
          </tr>
          <tr>
            <td>
                NANANANANA</td>
            <td>
                NANANANANA</td>
            <td>
                NANANANANA</td>
            <td>
                NANANANA</td>
          </tr>
        </table>
        <table class="texto" width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="30%">N&ordm; Atendimento </td>
            <td width="25%">Cor</td>
            <td width="15%">Idade</td>
            <td width="25%">Sexo</td>
          </tr>
          <tr>
            <td>
                NANANANANA</td>
            <td>
                NANANANANA</td>
            <td>
                NANANANANA</td>
            <td>
                NANANANA</td>
          </tr>
        </table>
</fieldset>      
		<fieldset>
			<legend>Descrição</legend>
    	    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="texto">
				<tr>
				  <td>
                      NANANANANANANANANANANANANA<br />
                      NANANANANANANANANA<br />
                      NANANANANANANANANANA<br />
                      NA<br />
                      NANA<br />
                      NANA<br />
                      NANANA<br />
                      NA<br />
                      NA<br />
                      NANANANANANANANANANANANA</td>
				</tr>
			</table>
		</fieldset>
		<fieldset>
            <legend>Exame F&iacute;sico</legend>
            <table class="texto" width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td width="12%">Ex. F&iacute;sico </td>
                <td colspan="5">
                    NANANANANANANANA</td>
              </tr>
              <tr>
                <td width="6%">PA:</td>
                <td width="24%">
                    NANANANANANANA</td>
                <td width="6%">FC:</td>
                <td width="19%">
                    NANANANANA</td>
                <td width="6%">FR:</td>
                <td width="39%">
                    NANANANANA</td>
              </tr>
              <tr>
                <td>T:</td>
                <td>
                    NANANANA</td>
                <td>Peso:</td>
                <td>
                    NANANANA</td>
                <td>Altura:</td>
                <td>
                    NANANANA</td>
              </tr>
            </table>
      </fieldset>
	</fieldset>
</asp:Content>

