...
<%
 set compras = abredb.Execute("SELECT pedido, clienteid, idcompra, datacompra, frete, totalcompra FROM compras WHERE clienteid='" & Cripto(Session("usuario"),true) & "' AND status <> 'Compra em Aberto' order by pedido desc")
