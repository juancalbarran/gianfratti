...
If Not IsEmpty(Request.Form("salvar")) Then
  ...
  If indice <> "" Then strQ = " SELECT * FROM departamentos WHERE " & indice

  objRS.Open strQ, objCon, , , &H0001
  If indice = "" Then objRS.Move Request.Form("recordno") - 1

  If objRS.Fields("departamento").properties("IsAutoIncrement") = False Then
    objRS("departamento") = Trim(Request.Form("departamento"))
  End If
  ...
  Set objCon = Nothing
%>
<B><font color="#CC0000">T&iacute;tulo alterado</font></B><BR>
Ot&iacute;tulo do departamento foi alterado com sucesso.<BR>
<BR>
<%
Else
  ...
  If indice = "" Then objRS.Move Request.Form("recordno") - 1
%>
<B><font color="#CC0000">Alterar Departamento</font></B><BR>
Altere os dados 
...
<%If objRS.Fields("departamento").properties("IsAutoIncrement") = False Then%>
<INPUT style="width=350" type="text" name="departamento" maxlength="255" value="<%=(objRS.Fields.Item("departamento").Value)%>" onKeyPress="desabilita_cor(this)"  df_verificar="sim" class=campos_formulario>
<%
Else
  Response.Write "<B>" & (objRS.Fields.Item("departamento").Value) & "</B>"
End If
%>
  </TD></TR>
</TABLE>
