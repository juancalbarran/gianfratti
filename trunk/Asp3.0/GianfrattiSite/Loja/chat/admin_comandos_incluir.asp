...
If Not IsEmpty(Request.Form) Then
  ...
  If campo_duplicado = false Then
    ...
    objRS.CursorType = 0
    objRS.LockType = 3
    strQ = "SELECT * FROM comandos Where 1 <> 1"
    ...
    Set objRS = Nothing
%>
<B><font color="#CC0000">Comando adicionado</font></B><BR>
O novo comando foi cadastrado com sucesso.<BR>
<BR>
<%
  Else
%>
<font color="#CC0000"><B>Atenção!</B></font><BR>
<BR>
...
Else
  ...
  objRS.Open strQ, objCon, , , &H0001
%>
<B><font color="#CC0000">Adicionar comando</font></B><BR>
...
<TABLE border=0 cellpadding=2 cellspacing=1 class=tabela_formulario>
  <TR class=titulo_campos><TD>Comando<BR>
    <SELECT style="width=350" name="tipo" df_verificar="sim" onChange="desabilita_cor(this)" class=campos_formulario>
      <OPTION value=""></OPTION>
      <OPTION value="1">Mensagem predefinida (Texto)</OPTION>
      <OPTION value="2">Exibir imagem (URL)</OPTION>
      <OPTION value="3">Exibir um link (URL)</OPTION>
      <OPTION value="4">Abrir janela do usuário (URL)</OPTION>
    </SELECT>
  </TD></TR>
  <TR class=titulo_campos><TD>Descricao do comando<br>
<%If objRS.Fields("descricao").properties("IsAutoIncrement") = False Then%>
<INPUT style="width=350" type="text" name="descricao" maxlength="50" value="" onKeyPress="desabilita_cor(this)"  df_verificar="sim" class=campos_formulario>
<%
Else
...
%>
  ...
  <TR class=titulo_campos><TD>atendente<BR>
  ...
  If Not objRS2.EOF Then
    While Not objRS2.EOF
	  If Trim(objRS2.Fields.Item("id").Value) <> "" Then
	    Response.Write "      <OPTION value='" & (objRS2.Fields.Item("id").Value) & "'>" & (objRS2.Fields.Item("atendente").Value) & "</OPTION>"
	  End If
      objRS2.MoveNext
    Wend
  End If
...
<%If objRS.Fields("valor").properties("IsAutoIncrement") = False Then%>
<INPUT style="width=350" type="text" name="valor" maxlength="255" value="" onKeyPress="desabilita_cor(this)"  df_verificar="sim" class=campos_formulario>
<%
Else
  Response.Write "<B>(Automático)</B>"
End If
