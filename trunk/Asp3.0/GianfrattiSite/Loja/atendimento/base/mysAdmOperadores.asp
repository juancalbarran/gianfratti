...
<table width="95%" cellpadding="1" align="center">
	...
	</td><td align="right"><a href="mysAdmOperadoresForm.asp"><img title="Adicionar Operador"  src="img/t_adicionar.gif" alt="" border="0"></a></td></tr>
...
strSQL = "SELECT op.id, op.nome AS NomeOP, dpt.nome AS NomeDpt, DateDiff(""s"", op.ping, now()) AS Ping FROM departamentos dpt, operadores op WHERE dpt.id = op.departamentoID  ORDER BY op.nome, dpt.nome"
...
If NOT rs.EOF Then
	...
	While Not rs.EOF
		...
		Response.Write "<td>&nbsp;"& rs("NomeOP") &"</td>"
		Response.Write "<td>&nbsp;"& rs("NomeDpt") &"</td>"
		Response.Write "<td>&nbsp;"
		If rs("Ping") > 5 OR isNull(rs("Ping")) Then
			Response.Write "<FONT COLOR=""#FF0000"">Offline</FONT>"
		Else
			Response.Write "<FONT COLOR=""#009900"">Online</FONT>"
		End If
		Response.Write "</td>"
		Response.Write "<td align=""center"">&nbsp;<a  href=""mysAdmOperadoresForm.asp?acao=Editar&ID="&rs("id")&""">Editar</a> | <a  href=""mysAdmOperadoresForm.asp?acao=Deletar&ID="&rs("id")&""">Deletar</a></td></tr>"
		Response.Write "<tr><td bgcolor=""#EDEDED"" colspan=""6"" height=""1""></td></tr>"
		rs.movenext
	Wend
Else
	Response.Write "<tr><td colspan=""6"" align=""center"">Nenhum operador cadastrado</td></tr>"
End If
