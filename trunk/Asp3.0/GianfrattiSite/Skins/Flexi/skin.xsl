...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<!-- VARIABLE DECALRATIONS - CHARS AND COLOURS   -->
	
	<!-- non-breaking space  -->
	<xsl:variable name="nbsp">&amp;nbsp;</xsl:variable>	
	
	<!-- colour group 1, dark  -->
	<xsl:param name="cg1d">#003366</xsl:param>
	<!-- colour group 1, mid   -->
	<xsl:param name="cg1m">#006699</xsl:param>
	<!-- colour group 1, light -->
	<xsl:param name="cg1l">#0099FF</xsl:param>
	<!-- colour group 1, wash  -->	
	<xsl:param name="cg1w">#66CCFF</xsl:param> 
	<!-- colour group 2, dark  --> 
	<xsl:param name="cg2d">#666666</xsl:param>
	<!-- colour group 2, mid   -->
	<xsl:param name="cg2m">#AAAAAA</xsl:param>
	<!-- colour group 2, light -->
	<xsl:param name="cg2l">#CCCCCC</xsl:param>
	<!-- colour group 2, wash  -->
	<xsl:param name="cg2w">#FFFFFF</xsl:param> 
	...
	<xsl:template name="sitetile_design">
		...
		<table><tr><td width="100%" style="color:{$cg1w}; filter:dropshadow(color={$cg2d}, offX=1, offY=1)"><font face="verdana" size="5" color="{$cg1d}"><b><xsl:copy-of select="$title" /></b></font></td></tr></table>
	...
	<xsl:template name="siteslogan_design">
		<xsl:param name="text"/>		
		<table><tr><td width="100%" style="color:{$cg1w}; filter:dropshadow(color={$cg1d}, offX=1, offY=1)"><font face="verdana" size="2" color="{$cg1m}"><xsl:copy-of select="$text"/></font></td></tr></table>
	</xsl:template>
	...
	<xsl:template name="pagetitle_design">
		<xsl:param name="title"/>
			...
			<font color="{$cg1d}" size="5"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
	...
	<xsl:template name="title_design">
		...
		<TABLE cellSpacing="0" cellPadding="0" width="100%" >
			<TR bgColor="{$cg2m}"><TD style="border-top: 2px {$cg2l} solid; border-bottom: 2px solid {$cg2d};" width="100%" nowrap="nowrap">
				<xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
				<b><font color="{$cg1d}" size="2">
			...
			</font></b><xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
			</TD></TR>
		</TABLE>			
	</xsl:template>
	...
	<xsl:template name="hightlightedtext_design">
		<xsl:param name="text"/>			
			<table bgcolor="{$cg2w}" border="0"><tr><td>
			       <!-- start uneditable zone -->
        			<xsl:choose>
        				<xsl:when test="contains($text, '&lt;')"><xsl:value-of select="$text" disable-output-escaping="yes"/></xsl:when>
        				<xsl:otherwise><xsl:copy-of select="$text" /></xsl:otherwise>
	...
	<xsl:template name="margin_design">
		...
		<table cellspacing="0" width="100%" style="border: 2px outset;" id="wp{$myid}">
	        
	        <xsl:if test="string-length(showtitle)=0 or showtitle='on'">
		        <tr> 
		        	<td width="100%" bgcolor="{$cg1d}"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
		          	<font color="{$cg2w}"><b>
		          		<!-- start uneditable zone -->
					<xsl:choose>
						<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
	        ...
	        <tr>
	        <td width="100%" colspan="4" bgcolor="{$cg2m}">
	        	<!-- start uneditable zone -->
				<xsl:choose>
	...
	<xsl:template name="rightmargin_design">
		...
		<table cellspacing="0" width="100%" style="border: 1px outset;" id="wp{$myid}">
	        <xsl:if test="string-length(showtitle)=0 or showtitle='on'">
		        <tr> 
		         	<td bgcolor="{$cg1d}" width="100%" ><xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
		          		<font color="{$cg2w}"><b><xsl:choose><xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when><xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise></xsl:choose></b></font>
		          	</td>
		          	<xsl:if test="$usertype='administrator'">
		          		<xsl:if test="$nextid">
		          			<form action="default.asp?id={$id}&amp;mnu={$mnu}" method="post">
	        ...
	        <tr>
	        	<td width="100%" colspan="2" bgcolor="{$cg2m}">
	        		<!-- start uneditable zone -->
				<xsl:choose>
					<xsl:when test="contains($body, '&lt;')"><xsl:value-of select="$body" disable-output-escaping="yes"/></xsl:when>
					<xsl:otherwise><xsl:copy-of select="$body" /></xsl:otherwise>
	...
	<xsl:template name="gateway_design">
		<table bgcolor="{$cg2w}" width="100%" height="100%">
			<tr>
				<td align="center">
					<!-- start uneditable zone -->
	...
	<xsl:template name="toolbar_global_design">
		<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0" style="border-top: 2px solid {$cg1l}; border-bottom: 2px solid {$cg1d};">
			<TR height="16"><TD bgColor="{$cg1m}" align="center" ><font color="{$cg1w}"><xsl:call-template name="toolbar_body_macro"/></font></TD></TR>
		</TABLE>				
	</xsl:template>
	...
	<xsl:template name="toolbar_item_design">
		<xsl:param name="content"/>
			...
			<xsl:choose>
				...
				<xsl:value-of select="$content" disable-output-escaping="yes"/></xsl:when>
				<xsl:otherwise><xsl:copy-of select="$content" /></xsl:otherwise>
			</xsl:choose>
			<!-- end uneditable zone -->
		<xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
		<xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
		<img src="skins/flexi/media/bullet.gif" width="6" height="6"/>
		<xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
		<xsl:value-of select="$nbsp" disable-output-escaping="yes"/>
	...
	<xsl:template name="headline_design">
		...
		<xsl:choose>
			...
			<xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
			<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
		</xsl:choose>
		<!-- end uneditable zone -->
		</b><br/><img src="skins/flexi/media/dot.gif" width="1" height="4"/><br/>				
