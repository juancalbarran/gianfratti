...
<xsl:output method="html" indent="no" encoding="iso-8859-1"/>	
	...
	<xsl:template name="sitetile_design">
		...
		<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="100%"><font face="{$general_font}" size="5" color="{$body_color}"><b><xsl:copy-of select="$title" /></b></font></td></tr></table>
	</xsl:template>
	...
	<xsl:template name="title_design">
		...
		<TABLE cellSpacing="0" cellPadding="0" width="100%" >
			<TR bgColor="{$toolbar_background}"><TD><xsl:value-of select="$nbsp" disable-output-escaping="yes"/><b><font color="{$toolbar_color}" size="2">
				<!-- start uneditable zone -->
				<xsl:choose>
					<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
					<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
