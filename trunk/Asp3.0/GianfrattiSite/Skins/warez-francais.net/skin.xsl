...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="title_design">
		...
		<tr> 
			<td width="1"><img height="12" src="skins/warez-francais.net/media/fleche_violet.gif" width="12"/></td>
			<td class="rub" width="1"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/></td>
			<td bgcolor="#f8c555" CLASS="rubflech4"> <font color="#662e86"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
			<!-- start uneditable zone -->
			<xsl:choose>
				<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
				<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
	...
	<xsl:template name="margin_design">
		...
		<table bgcolor="white" border="0" cellpadding="0" cellspacing="0" width="100%">
	        <tr> 
	          <td><img height="12" src="skins/warez-francais.net/media/fleche_orange.gif" width="13"/></td>
	          <td class="rub" width="100%" bgcolor="#662e86"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
	          	<!-- start uneditable zone -->
			<xsl:choose>
				<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
				<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
	...
	<xsl:template name="rightmargin_design">
		...
		<table bgcolor="white" border="0" cellpadding="0" cellspacing="0" width="100%">
	        <tr> 
	          <td><img height="12" src="skins/warez-francais.net/media/fleche_violet.gif" width="13"/></td>
	          <td><img height="1" src="skins/warez-francais.net/media/dot.gif" width="1"/></td>
	          <td class="rub" width="100%" bgcolor="#662e86"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
	          	<!-- start uneditable zone -->
			<xsl:choose>
				<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
	...
	<xsl:template name="toolbar_global_design">
		<div align="center"><center>
		  <table border="0" width="100%" cellspacing="0" cellpadding="0">		    
		    	<tr> 
				<td colspan="2"> 
			      <div align="center" class="menu"><xsl:call-template name="toolbar_body_macro"/></div>
			    </td>
			</tr>    
		  </table>
		  </center></div>		
	...
	<xsl:template name="headline_design">
		...
		<TABLE WIDTH="225" BORDER="0" CELLSPACING="0" CELLPADDING="0">
			<TR>
				<TD><IMG src="skins/warez-francais.net/media/fleche_orange.gif" WIDTH="13" HEIGHT="12"/></TD>
				<TD WIDTH="212" CLASS="rubflech3"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
					<!-- start uneditable zone -->
					<xsl:choose>
						<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
						<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
