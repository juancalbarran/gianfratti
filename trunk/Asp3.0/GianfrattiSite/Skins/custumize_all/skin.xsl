...
<xsl:output method="html" indent="no" encoding="iso-8859-1"/>	
	...
	<xsl:template name="sitetile_design">
		...
		<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="100%"><font face="{$general_font}" size="5" color="{$headertext_color}"><b><xsl:copy-of select="$title" /></b></font></td></tr></table>
	</xsl:template>
	...
	<xsl:template name="siteslogan_design">
		<xsl:param name="text"/>		
		<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="100%"><font color="{$headerslogan_color}"><xsl:copy-of select="$text"/></font></td></tr></table>
	</xsl:template>
	...
	<xsl:template name="hightlightedtext_design">
		<xsl:param name="text"/>			
			<table bgcolor="{$highlight_background}" width="100%" style="border: 1px solid {$highlight_border}"><tr><td>
			       <!-- start uneditable zone -->
        			<xsl:choose>
        				<xsl:when test="contains($text, '&lt;')"><xsl:value-of select="$text" disable-output-escaping="yes"/></xsl:when>
        				<xsl:otherwise><xsl:copy-of select="$text" /></xsl:otherwise>
	...
	<xsl:template name="margin_design">
		...
		<table cellspacing="0" width="100%" style="border: 1px outset;" id="wp{$myid}">
	        ...
	        <xsl:if test="string-length(showtitle)=0 or showtitle='on'">
		        <tr> 
		        	<td width="100%" bgcolor="{$menuheadleft_background}"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
		          	<font color="{$menuheadleft_color}"><b>
		          		<!-- start uneditable zone -->
					<xsl:choose>
						<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
						<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
	...
	<xsl:template name="rightmargin_design">
		...
		<table cellspacing="0" width="100%" style="border: 1px outset;" id="wp{$myid}">
	        <xsl:if test="string-length(showtitle)=0 or showtitle='on'">
		        <tr> 
		         	<td bgcolor="{$menuheadright_background}" width="100%" ><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
		          		<font color="{$menuheadright_color}"><b><xsl:choose><xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when><xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise></xsl:choose></b></font>
		          	</td>		          	
		        </tr>
	        </xsl:if>
	...
	<xsl:template name="toolbar_global_design">
		<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0" style="border-top: 1px solid {$toolbar_border}; border-bottom: 1px solid {$toolbar_border};">
			<TR height="16"><TD bgColor="{$toolbar_background}" align="right"><font color="{$toolbar_color}"><xsl:call-template name="toolbar_body_macro"/></font></TD></TR>
		</TABLE>				
	</xsl:template>
