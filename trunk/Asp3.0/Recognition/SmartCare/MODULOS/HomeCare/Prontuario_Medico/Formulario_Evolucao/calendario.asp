...
Function BuscaDataBancoDados(Data)
	...
	SQL	=	"SELECT  CONVERT(CHAR, Data_Evolucao, 103) AS Data_Evolucao " &_
			"FROM HomeCare_Atendimento_Paciente_Evolucao " &_ 
			"WHERE (CONVERT(CHAR, Data_Evolucao, 103) = '"&Data&"') " 
			Set rs = server.createobject("adodb.recordset")
			Rs.open SQL, CONN, 3
				IF not rs.eof then 'Verifica se existem registros
				
					BuscaDataBancoDados = Rs("Data_Evolucao")
...
<TABLE width="100%" height="102" BORDER=0 CELLPADDING=0 CELLSPACING=0>
  <TR>
    <TD valign="top"> 
      <TABLE width="63%" height="102" BORDER=1 CELLPADDING=1 CELLSPACING=0 BGCOLOR=#99CCFF>
        <TR>
		<TD BGCOLOR=#000099 ALIGN="center" COLSPAN=7>
			<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0>
				<TR>
					<TD ALIGN="right"><A HREF="historico.asp?id_Atendimento=<%=Request("id_Atendimento")%>&date=<%= ArrumaData(SubtractOneMonth(dDate)) %>"><FONT COLOR=#FFFF00 SIZE="1">&lt;&lt;</FONT></A></TD>
					<TD ALIGN="center"><FONT COLOR=#FFFF00 size="1"><B><%= Ucase(MonthName(Month(dDate))) & "  " & Year(dDate) %></B></FONT></TD>
					<TD ALIGN="left"><A HREF="historico.asp?id_Atendimento=<%=Request("id_Atendimento")%>&date=<%= ArrumaData(AddOneMonth(dDate)) %>"><FONT COLOR=#FFFF00 SIZE="1">&gt;&gt;</FONT></A></TD>
				</TR>
			</TABLE>
		</TD>
	</TR>
...
'************ Fim do codigo de comparação de datas
	...
	If iCurrent = Day(dDate) Then
		Response.Write vbTab & vbTab & "<TD ALIGN='center' BGCOLOR=#00FFFF><FONT SIZE=""1""><B>" & "<a href='historico.asp?id_Atendimento="&Request("id_Atendimento")&"&date=" & StrZero(iCurrent,2) & "/" & StrZero(Month(dDate),2) & "/" & Year(dDate) & "'>"&iCurrent&"</a></B></FONT></TD>" & vbCrLf
	Else
		Response.Write vbTab & vbTab & "<TD ALIGN='center' "&Cor_Destaque& "><A HREF=""historico.asp?id_Atendimento="&Request("id_Atendimento")&"&date=" & StrZero(iCurrent,2) & "/" & StrZero(Month(dDate),2) & "/" & Year(dDate) & """><FONT SIZE=""1"">" & iCurrent & "</FONT></A></TD>" & vbCrLf
	End If
