...
function RasterTwain_AquirePageEvent(pBitmap)
{
    ...
    RasterView.Raster.RasterUNK.InsertBitmapListItem(-1, pBitmap);    
    RasterView.Raster.RasterUNK.BitmapListIndex = RasterView.Raster.RasterUNK.BitmapListCount - 1;
    ...
    if(hWnd != null)
        ...
        alert("Window Failed to Create! Is your popup blocker disabled?");
}
...
{
   if (RasterView.Raster.RasterUNK.BitmapListIndex - 1 > -1 )
   {
        RasterView.Raster.RasterUNK.BitmapListIndex = RasterView.Raster.RasterUNK.BitmapListIndex - 1;
   }   
}
function Next_onclick() 
{
   if (RasterView.Raster.RasterUNK.BitmapListIndex + 1 < RasterView.Raster.RasterUNK.BitmapListCount)
   {
        RasterView.Raster.RasterUNK.BitmapListIndex = RasterView.Raster.RasterUNK.BitmapListIndex + 1;
   }   
}
...
<font face="Arial" size="2"><strong>Scan Upload Demo using LEAD COM Objects on the Server</strong></font></center>
      ...
      <CENTER><input id="Prev" style="LEFT: 318px; TOP: 121px" type="button" value="Prev" name="Prev" LANGUAGE="javascript" onclick="return Prev_onclick()"></CENTER>
      <CENTER><input id="Next" style="LEFT: 318px; TOP: 121px" type="button" value="Next" name="Next" LANGUAGE="javascript" onclick="return Next_onclick()"></CENTER>
      <CENTER>&nbsp;</CENTER>
<%
	Dim pathInfo
	Dim bFlag
