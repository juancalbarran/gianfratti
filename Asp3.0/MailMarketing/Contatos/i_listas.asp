...
<tr>
	<td>
		...
		%>
		<form method="post" action="p_contatos.asp?opcao=<%=Opcao%>&acao=<%=Acao%>" onSubmit="return(document.frmPesquisa.pesquisa.value.toString().trim() == '')?false:true;" name="frmPesquisa">
			<tr>
				<td><img src="../Shared/img/spacer.gif" width="1" height="5"></td>
			</tr>
			<tr>
				<td>Digite o Nome do Contato ou E-mail:</td>
			</tr>
			<tr>
...
<tr>
	<td>
<%
sSQL = "SELECT Contato_CD As [Código], Contato_NM As [Nome], Contato_Email As [E-mail]  FROM CONTATOS Where Deletado=0 "

if pesquisa <> "" Then

	pesquisa = Replace(pesquisa, "'","''")
	
	sSQL = sSQL & " and ( Contato_NM Like '%" & pesquisa & "%' Or Contato_Email Like '%" & pesquisa & "%' )"
End If

sSQl = sSQL & " Order By Contato_NM,  Contato_eMail"	
%>
<%Call RetonaLista(sSQL, "50", "p_contatos.asp?pesquisa=" & pesquisa & "&opcao=" & opcao & "&acao=" & acao , sOpcao, "p_contatos.asp?opcao=" & opcao & "&acao=2")%>
	</td>
</tr>
</table>
