...
'Tira letras do campo CPF/CNPJ
strCpf = SoNumeros(strCPF)

'Tira letras do campo CEP
strCep = SoNumeros(strCep)

strFone = SoNumeros(strFone)

'Tira letras do campo RG/Inscricao
if Ucase(strRG)<>"ISENTO" then
  strRg = SoNumeros(strRg)
End if
...
bValido = ValidaCPF_CNPJ(strCPF,len(strCPF))
...
if len(strSenha) > 10 OR len(strSenha) < 5 OR cdate(strNascano) < cdate(anoantigo1) OR cdate(strNascano) > cdate(anonovo1) OR strEmail = "" OR instr(strEmail, "@") = 0 Or instr(strEmail, ".") = 0 OR strEmail = "sim@." OR strSenha = "" OR strNome = "" OR strNascdia = "" OR strNascano = "" OR strNascmes = "" OR strRg = "" OR strEndereco = "" OR strBairro = "" OR strCidade = "" OR strCep = "" OR strFone = "" OR strCpf = "" OR bValido = "falso" then

'Valida o CPF e retorna a transação na loja
 ElCpf = request.form("cpf")
 if bValido = "falso" then
   strCpf = ElCpf
 end if
 ElCPF = SoNumeros(ElCPF)

 if ElCpf = "" then
  erro4 = "<font style=font-size:11px color=red> - " & strLg141 & "</font>"
 end if

 bValido = ValidaCPF_CNPJ(ElCPF,len(ElCPF))

 if  bValido="falso" then
  erro4 = "<font style=font-size:11px color=red> - " & strLg143 & "</font>"
 end if
...
strCep = "-"
	...
	if strCep = "" then
	erro9 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
	else
		if isnumeric(strCep) = false then
		erro9 = "<font style=font-size:11px; color=red> - " & strLg144 & "</font>"
		else
		erro9 = ""
		end if
	end if
...
erro3 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
	if cdate(strNascano) < cdate(anoantigo) OR cdate(strNascano) > cdate(anonovo) then
	erro3 = "<font style=font-size:11px; color=red> - " & strLg144 & "</font>"
	else
	erro3 = ""
	end if
end if
...
'Valida o telefone
if strFone = "" then
erro10 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
	if isnumeric(strFone) = false then
	erro10 = "<font style=font-size:11px; color=red> - " & strLg144 & "</font>"
	else
	erro10 = ""
	end if
end if
...
if strCidade = "" then
erro8 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
erro8 = ""
end if

if strBairro = "" then
erro7 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
erro7 = ""
end if

if strEndereco = "" then
erro6 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
erro6 = ""
end if

if strRg = "" then
erro5 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else

	'Valida o RG
	if isnumeric(strRG) = false then
	erro5 = "<font style=font-size:11px; color=red> - " & strLg144 & "</font>"
	else
	erro5 = ""
	end if
end if
...
erro = "<font style=font-size:11px; color=red> - " & strLg144 & "</font>"
else
	'*** Erros comuns de clientes...
	strEmail=replace(strEmail,"hotmail.com.br","hotmail.com")
	strEmail=replace(strEmail,"www.","")
erro = ""
end if
if strEmail = "sim@." then
erro = "<font style=font-size:11px; color=red> - " & strLg145 & "</font>"
strEmail = ""
end if
if strSenha = "" then
erro1 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
erro1 = ""
end if
if len(strSenha) > 10 then
erro1 = "<font style=font-size:11px; color=red> - " & strLg234 & "</font>"
end if
if len(strSenha) < 5 then
erro1 = "<font style=font-size:11px; color=red> - " & strLg235 & "</font>"
end if
if strNome = "" then
erro2 = "<font style=font-size:11px; color=red> - " & strLg141 & "</font>"
else
erro2 = ""
end if
				   ...
				   <table border="0" cellspacing="4" cellpadding="4" width=590><tr><td><font face="<%=fonte%>" style=font-size:11px;color:000000> <a href=default.asp style=text-decoration:none; onMouseOut="window.status='';return true;" onMouseOver="window.status='Home';return true;"><b>Home</b></a> » <%=strLg5%><br><br><div align=left> <table border=0 cellspacing=0 width=100% cellpadding=0><tr><td height=5></td></tr><tr><td height=1 bgcolor=<%=cor3%>></td></tr><tr><td height=5></td></tr></table></div><font style=font-size:10px><%=strLg122%>
					...
					<form action=registros.asp method=post name="registro1" onMouseOver="javascript: paises();">
					 <input type=hidden name=tipo value=cadastrar>
					  <tr><td colspan=2><br><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><b><%=strLg289%><table border=0 cellspacing=0 width=100% cellpadding=0><tr><td height=5></td></tr><tr><td height=1 bgcolor=<%=cor3%>></td></tr><tr><td height=5></td></tr></table></td></tr>
					  <tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg231%></td><td><input type=text name=email style=font-family:<%=fonte%>;font-size:11px;color:#000000; size=30 value="<%=strEmail%>" maxlength="50"><%=erro%></td></tr>
					  ...
					  <tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg118%></td><td><input type=text size=40 name=nomecompleto style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strNome%>" maxlength="30"><%=erro2%></td></tr>
...
<option value="Jan">Jan <option value="Fev">Fev<option value="Mar">Mar<option value="Abr">Abr<option value="Mai">Mai <option value="Jun">Jun <option value="Jul">Jul<option value="Ago">Ago<option value="Set">Set<option value="Out">Out<option value="Nov">Nov<option value="Dez">Dez</option></select><b>/</b><input style="font-family:<%=fonte%>;font-size:11px;color:#000000;" maxLength="4" size="5" name="nascano" value="<%=strNascano%>"><%=erro3%></font></td></tr>
					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg116%><sup><font size="3" color="#FF0000">*</font></sup></td><td><input  type=text size=19 name=cpf style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strCpf%>" maxlength="50"><%=erro4%></td></tr>
					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg115%><sup><font size="3" color="#FF0000">*</font></sup></td><td><input type=text size=19 name=rg style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strRg%>" maxlength="50"><%=erro5%></td></tr>
					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg114%></td><td><input type=text size=35 name=endereco style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strEndereco%>" maxlength="100"><%=erro6%></td></tr>
					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg113%></td><td><input type=text size=35 name=bairro style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strBairro%>" maxlength="50"><%=erro7%></td></tr>
					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg112%></td><td><input type=text size=35 name=cidade style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strCidade%>" maxlength="50"><%=erro8%></td></tr>
...
%>
					...
					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg82%></td><td><input type=text size=13 name=cepzz style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="" maxlength="8"></td></tr>
...
<%end if%>
							...
							<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;>CEP:</td><td><input type=text size=13 name=cepzz style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%= strCep %>" maxlength="8"><%= erro9 %></td></tr>
...
<option value="Sudão">Sudão</option><option value="Suécia">Suécia</option><option value="Suíça">Suíça</option><option value="Suriname">Suriname</option><option value="Tadjiquistão">Tadjiquistão</option><option value="Tailândia">Tailândia</option><option value="Taiti">Taiti</option><option value="Taiwan">Taiwan</option><option value="Tanzânia">Tanzânia</option><option value="Terra de Francisco José - Rússia">Terra de Francisco José - Rússia</option><option value="Togo">Togo</option><option value="Tonga">Tonga</option><option value="Trinidad e Tobago">Trinidad e Tobago</option><option value="Tristão da Cunha - Reino Unido">Tristão da Cunha - Reino Unido</option><option value="Tunísia">Tunísia</option><option value="Turcomenistão">Turcomenistão</option><option value="Turquia">Turquia</option><option value="Tuvalu">Tuvalu</option><option value="Ucrânia">Ucrânia</option><option value="Uganda">Uganda</option><option value="Uruguai">Uruguai</option><option value="Uzbequistão">Uzbequistão</option><option value="Vanuatu">Vanuatu</option><option value="Venezuela">Venezuela</option><option value="Vietnã">Vietnã</option><option value="Zaire">Zaire</option><option value="Zâmbia">Zâmbia</option><option value="Zimbábue">Zimbábue</option></select></td></tr>
<%end if%>
	  	  					<tr><td><font style=font-family:<%=fonte%>;font-size:11px;color:#000000;><%=strLg109%></td><td><input type=text size=35 name=fone style=font-family:<%=fonte%>;font-size:11px;color:#000000; value="<%=strFone%>" maxlength="50"> <font style=font-family:<%=fonte%>;font-size:9px;color:#000000;><%=erro10%></td></tr>
									 ...
									 <tr><td colspan="2"><br><font style=font-family:<%=fonte%>;font-size:11px;color:#FF0000;><sup><font size="3">*</font></sup> tipo de informações solicitadas se for pessoa jurídica</tr>
...
'Corpo do e-mail de envio de cadastro
...
strMensagem = strMensagem & "      style='FONT-SIZE: 11px'><B>Cadastro de "&strNome&"</B></FONT>" 
strMensagem = strMensagem & "      </FONT>"
strMensagem = strMensagem & "    <TD align=right colSpan=2><B><FONT face="&fonte&"><FONT color=#000000" 
strMensagem = strMensagem & "      style='FONT-SIZE: 11px'>Data: </B>"&day(now)&"/"&month(now)&"/"&year(date)&" </FONT>"
...
strMensagem = strMensagem & "      <DIV><FONT face="&fonte&" style='FONT-SIZE: 11px'>Obrigado por Cadastrar na" 
strMensagem = strMensagem & "      "&nomeloja&"!<BR>Esta é a confirmação de que seu Cadastro"
strMensagem = strMensagem & "      </STRONG>foi realizado com sucesso.<BR><BR>Nas suas futuras compras no nosso site, os seus dados de login de acesso são:<br><br><strong>Email:</strong> "&strEmail&"<br><strong>Senha:</strong> "&strSenha&"</FONT></DIV>"
...
strMensagem = strMensagem & "      <DIV align=left><FONT face="&fonte&" style='FONT-SIZE: 11px'><STRONG>Seus" 
strMensagem = strMensagem & "      Dados Cadastrais</STRONG></FONT></FONT></FONT><FONT face="&fonte&" "
strMensagem = strMensagem & "      style='FONT-SIZE: 11px'><BR><BR></DIV></DIV></FONT></TD></TR>"
strMensagem = strMensagem & "  <TR>"
strMensagem = strMensagem & "    <TD colSpan=3><FONT face="&fonte&"><FONT color=#000000 style='COLOR: #000000; FONT-SIZE: 11px'><B>E-mail:" 
strMensagem = strMensagem & "      </B>"&strEmail&"<BR><B>Senha: </B>"&strSenha&" "
strMensagem = strMensagem & "      <BR><B>Nome:</B> "&strNome&" </FONT></FONT></TD>"
strMensagem = strMensagem & "      <TD colSpan=2><FONT face="&fonte&"><FONT color=#000000 style='COLOR: #000000; FONT-SIZE: 11px'><B>Nascimento:</B>" 
strMensagem = strMensagem & "      "&strNascdia&"-"&strNascmes&"-"&strNascano&" <BR><B>CPF:</B> "&strCpf&" <BR><B>RG:</B> "&strRg&"</TR> "
strMensagem = strMensagem & "    <TR>"
strMensagem = strMensagem & "      <TD colSpan=3><FONT face="&fonte&"><FONT color=#000000 style='COLOR: #000000; FONT-SIZE: 11px'><B>Endereço:" 
...
strMensagem = strMensagem & "      <DIV align=left><FONT face="&fonte&"><FONT style='FONT-SIZE: 11px'>" 
strMensagem = strMensagem & "      </DIV>"
strMensagem = strMensagem & "      <DIV align=left><FONT face="&fonte&"><FONT style='FONT-SIZE: 11px'>" 
strMensagem = strMensagem & "    <TD colSpan=4><FONT face="&fonte&"><B><FONT style=font-size:11px>LEMBRE-SE de que os dados acima devem estar corretos e completos para não haver"
strMensagem = strMensagem & "      demora e dificuldade no atendimento dos seus futuros pedidos.<BR><BR> "

if session("modo_entrega")="motoboy" or session("modo_entrega")="sedex" or session("modo_entrega")="encomenda" then
strMensagem = strMensagem & "      Para evitar extravios e garantir que a entrega seja feita corretamente, o" 
strMensagem = strMensagem & "      pedido somente será deixado no endereço solicitado mediante a assinatura de quem for recebê-lo."
end if
...
strMensagem = strMensagem & "      serão informados para o email ou telefone informado acima.</FONT></DIV>"
end if

strMensagem = strMensagem & "      <BR><BR><B>Atenciosamente,<BR>Atendimento ao" 
strMensagem = strMensagem & "      Cliente.</B><BR> Equipe"
...
'*** Feito isso pois o comando ALTER TABLE `clientes` ADD `contato` VARCHAR(50) deixa setado Nao Permitir Comprimento Zero
if strContato="" or IsNull(strContato) then 
strContato=" "
end if

'response.write "<br>strContato :"&strContato
'response.end

'strContato2=strContato&"a "
'strContato=strContato2

if strOutropais = "Sim" then
textosql = "INSERT INTO clientes (datacad, nome, email, senha, cpf, rg, contato, nascimento, endereco, bairro, cidade, pais, tel, status) VALUES ('"&day(now)&"/"&month(now)&"/"&year(now)&"', '"&Cripto(strNome,True)&"', '"&Cripto(strEmail,True)&"', '"&Cripto(strSenha,true)&"', '"&Cripto(strCpf,true)&"', '"&Cripto(strRg,true)&"', '"&Cripto(strContato,True)&"', '"&Cripto(strNascdia&"/"&strNascmes&"/"&strNascano,true)&"', '"&Cripto(strEndereco,true)&"', '"&Cripto(strBairro,true)&"', '"&Cripto(strCidade,true)&"', '"&Cripto(strPais,true)&"', '"&Cripto(strFone,true)&"', 'Ativo');"
else
textosql = "INSERT INTO clientes (datacad, nome, email, senha, cpf, rg, contato, nascimento, endereco, bairro, cidade, estado, cep, pais, tel, status) VALUES ('"&day(now)&"/"&month(now)&"/"&year(now)&"', '"&Cripto(strNome,true)&"', '"&Cripto(strEmail,true)&"', '"&Cripto(strSenha,true)&"', '"&Cripto(strCpf,true)&"', '"&Cripto(strRg,true)&"', '"&Cripto(strContato,True)&"', '"&Cripto(strNascdia&"/"&strNascmes&"/"&strNascano,true)&"', '"&Cripto(strEndereco,true)&"', '"&Cripto(strBairro,true)&"', '"&Cripto(strCidade,true)&"', '"&Cripto(strEstado,true)&"', '"&Cripto(strCep,true)&"', '"&Cripto(strPais,true)&"', '"&Cripto(strFone,true)&"', 'Ativo');"
end if

session("textosql_registros") = textosql
...
<%
EnviaEmail Application("HostLoja"), Application("ComponenteLoja"), emailloja, "", session("usuario"), "Confirmação de Cadastro na "&nomeloja&"!", strMensagem
EnviaEmail Application("HostLoja"), Application("ComponenteLoja"), emailloja, "", emailloja, "Novo Cadastro de Cliente na "&nomeloja&"!", strMensagem

%>
  			  <table><tr><td align="left" valign="top">
			  				 <table border="0" cellspacing="4" cellpadding="4" width=590><tr><td><font face="<%=fonte%>" style=font-size:11px;color:000000><a href=default.asp style=text-decoration:none; onMouseOut="window.status='';return true;" onMouseOver="window.status='Home';return true;"><b>Home</b></a> » <%=strLg5%><br><br><table border=0 cellspacing=0 width=100% cellpadding=0><tr><td height=5></td></tr><tr><td height=1 bgcolor=<%=cor3%>></td></tr><tr><td height=5></td></tr></table><br><br><p align=center><font style=font-size:17px; color=#000000><b><%=strLg236%></b></font></p><p align=center><%=strLg237%> <b><%=strnome%></b>, <%=strLg238%>&nbsp;<%=nomeloja%>.<br></p>

<p align=center>
<% 
intOrderID = cstr(Session("orderID"))
set carrinho = abredb.Execute("SELECT idprod, quantidade FROM pedidos WHERE idcompra='" & intOrderID & "'")
if not carrinho.eof then
 %>
<a href=fechapedido.asp?compra=ok><img src="<%=dirlingua%>/imagens/fecha.gif" onMouseOut="window.status='';return true;" onMouseOver="window.status='<%=strLg51%>';return true;" border=0></a>
<%
End If
carrinho.Close
set carrinho = Nothing
%>
<br><br><a href=default.asp><img src="<%=dirlingua%>/imagens/continuar.gif" onMouseOut="window.status='';return true;" onMouseOver="window.status='<%=strLg68%>';return true;" border=0></a>
</p>

 <br><table border=0 cellspacing=0 width=100% cellpadding=0><tr><td height=5></td></tr><tr><td height=1 bgcolor=<%=cor3%>></td></tr><tr><td height=5></td></tr></table></td></tr>
							 </table></td></tr>
			  </table>
			  <!-- #include file="baixo.asp" -->
