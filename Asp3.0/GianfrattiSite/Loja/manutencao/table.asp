...
If TableExists(table) = False Or IsNumeric(table) = True Then
	'Invalid Table
	strError = "Nome da tabela é inválido. " & _
	"Volte, se o erro persistir contate o administrador"
...
Sub DisplayTable
    ...
    [ <a href="record.asp?action=addrecord&table=<%= table %>&sql=bypassfilter">
        <b>Insert</b></a> ]&nbsp;&nbsp;&nbsp;
    [ <a href="javascript:ConfirmLink('table.asp?action=empty&table=<%= table %>', 'Você deseja limpar essa tabela?')">
         <b>Empty</b></a> ]&nbsp;&nbsp;&nbsp;
    [ <a href="javascript:ConfirmLink('table.asp?action=drop&table=<%= table %>', 'Você deseja deletar esta tabela?')">
         <b>Drop</b></a> ]
	</p>
		...
		<table border="0" id="memgroup" width="100%">
			...
			%>
				...
				<%
					...
					<td id="tdrow2">
						<a href="javascript:ConfirmLink('table.asp?action=dropfield&field=<%= field.Name %>&table=<%= table %>', 'Você deseja deltar esse campo?\n\nVerifique se o campo é indexado.')">
						Deletar</a>
					</td>
					<td id="tdrow2">
					...
					<td id="tdrow2">
						<a href="javascript:ConfirmLink('table.asp?action=addindex&subaction=primary&table=<%= table %>&field=<%= field.Name %>', 'Verifique se a tabela contém campo com chave primaria.\nContinuar?')">
						Chave Primária</a>
					</td>
				</tr>
				<%
				...
				<table border="0" id="memgroup">
			        ...
			        For Each index In db.objADOX.Tables(table).Indexes
						...
						<tr>
							...
							<td id="tdrow2" rowspan="1">
								<a href="javascript:ConfirmLink('table.asp?action=dropindex&index=<%= index.Name %>&table=<%= table %>', 'Você deseja deletar?')">Deletar</a>
							</td>
							<td id="tdrow2">
								<%= index.Columns(0) %>
							</td>
    ...
    <!-- Deletes the table -->
    <li>
        <a href="javascript:ConfirmLink('table.asp?action=drop&table=<%= table %>', 'Deseja deletar essa tabela?')">
            Deletar tabela</a>
    </li>
