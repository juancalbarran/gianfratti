...
<body class="texto" bgcolor="white" leftmargin="10" topmargin="10" bottommargin="10" rightmargin="10">
<B><font color="#CC0000">Atendimentos</font></B><BR>
Abaixo meu hist&oacute;rico de atendimentos:<br><br>
<%
...
Else
  ...
  ComandoSQL = ComandoSQL & "AND atendente = " & atendente
...
objRS_atendimentos.PageSize = RegPorPag
  ...
  Response.Write "  <form name='form_pesquisar' method='post' action='" & Request.ServerVariables("SCRIPT_NAME") & "?atendente=" & atendente & "'>"
  Response.Write "  <tr>"
  Response.Write "    <td>"
...
If Not objRS_atendimentos.EOF Then
  ...
  For Cont = 1 to objRS_atendimentos.PageSize
    ...
    Response.Write "    <td width='30%' nowrap><font color='#3E6497'><b>" & objRS_atendimentos("usuario") & "</b></font><br>" & objRS_atendimentos("email") & "&nbsp;</td>"
  ...
  LinksNavegacao()
Else
  Response.Write "&nbsp;&nbsp;Nenhum atendimento"
End If
...
If PagAtual > 1 Then
  Response.Write "<A href=""" & Request.ServerVariables("SCRIPT_NAME") & "?atendente=" & atendente & "&PagAtual=" &  PagAtual &"&VarPagMax=" & VarPagMax & "&NumPagMax=" & NumPagMax & "&Submit=Anterior&string_busca=" & Server.URLEncode(Request("string_busca")) & "&campo_busca=" & Server.URLEncode(Request("campo_busca"))  & """ style='text-decoration=none'><font color=blue>&lt; Anterior</font></A>"
End If
Response.Write "</TD><TD width=33% align=middle vAlign=top noWrap>"
If NumPagMax - VarPagMax <> 0 then
  Response.Write "&nbsp;<A href=""" & Request.ServerVariables("SCRIPT_NAME") & "?atendente=" & atendente & "&PagAtual=" & NumPagMax - VarPagMax & "&VarPagMax=" & VarPagMax & "&NumPagMax=" & NumPagMax - VarPagMax & "&Submit=Menos&string_busca=" & Server.URLEncode(Request("string_busca")) & "&campo_busca=" & Server.URLEncode(Request("campo_busca")) & """ style='text-decoration=none'><font color=blue>&lt;&lt;<font></A>&nbsp;&nbsp;"
End If
for i = NumPagMax - (VarPagMax - 1) to NumPagMax
  If i <= TotPag then
    If i <> CInt(PagAtual) then
      Response.Write "&nbsp;<A href=""" & Request.ServerVariables("SCRIPT_NAME") & "?atendente=" & atendente & "&PagAtual=" & PagAtual & "&VarPagMax=" & VarPagMax & "&NumPagMax=" & NumPagMax & "&Submit=" & i & "&string_busca=" & Server.URLEncode(Request("string_busca")) & "&campo_busca=" & Server.URLEncode(Request("campo_busca")) & """ style='text-decoration=none'><font color=blue>" & i & "</font></A>&nbsp;"
    Else
...
If NumPagMax  < TotPag then
  Response.Write "&nbsp;&nbsp;<A href=""" & Request.ServerVariables("SCRIPT_NAME") & "?atendente=" & atendente & "&PagAtual=" & NumPagMax + 1 & "&VarPagMax=" & VarPagMax & "&NumPagMax=" & NumPagMax + VarPagMax & "&Submit=Mais&string_busca=" & Server.URLEncode(Request("string_busca")) & "&campo_busca=" & Server.URLEncode(Request("campo_busca")) & """ style='text-decoration=none'><font color=blue>&gt;&gt;</font></A>"
End If
Response.Write "</TD><TD width=33% align=left vAlign=top noWrap>"
If PagAtual <> TotPag Then
  Response.Write "&nbsp;&nbsp;<A href=""" & Request.ServerVariables("SCRIPT_NAME") & "?atendente=" & atendente & "&PagAtual=" & PagAtual & "&VarPagMax=" & VarPagMax & "&NumPagMax=" & NumPagMax & "&Submit=Proxima&string_busca=" & Server.URLEncode(Request("string_busca")) & "&campo_busca=" & Server.URLEncode(Request("campo_busca")) & """ style='text-decoration=none'><font color=blue>Proxima &gt;</font></A>"
End If
