...
// end insert by lvn
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
	...
	var myText = showModalDialog(_editor_url + "popups/insert_easylink.html", editdoc, "resizable: no; help:no; status: no; scroll: no; ");
	if (myText) {
...
showModalDialog(_editor_url + "popups/insert_image2.html", editdoc, "resizable: no; help:no; status: no; scroll: no; ");
    }
	

// insert form
  else if (cmdID.toLowerCase() == 'inputform') {
	var myText = showModalDialog(_editor_url + "popups/insert_form.html","","resizable: no; help: no; status: no; scroll: no; ");
	if (myText) {
		editor_insertHTML(objname, unescape( myText) );
	}
  }
