...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="sitetile_design">
		...
		<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="100%" style="color:#6a7c52; filter:dropshadow(color=#aaaaaa, offX=3, offY=2)"><font face="georgia" size="7"><b><i>..:: <xsl:copy-of select="$title" /></i></b></font></td></tr></table>
		<!--<font face="verdana" size="7" color="#6a7c52"><b><i><xsl:copy-of select="$title" /></i></b></font><br/>-->
	</xsl:template>
	...
	<xsl:template name="siteslogan_design">
		<xsl:param name="text"/>		
		<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="100%" style="color:#000000; padding-left:100px; padding-bottom:3px; filter:dropshadow(color=#aaaaaa, offX=1, offY=1)"><font face="georgia" color="#000000" size="3"><xsl:copy-of select="$text"/></font></td></tr></table>
	</xsl:template>
	...
	<xsl:template name="pagetitle_design">
		<xsl:param name="title"/>
			...
			<font color="{$pagetitle_color}" size="5">
	...
	<xsl:template name="title_design">
		...
		<TABLE bgColor="{$title_background}" cellSpacing="0" cellPadding="0" width="100%"  style="BORDER-BOTTOM: #333333 2px solid;BORDER-LEFT: #b2bfa0 2px solid;BORDER-RIGHT: #333333 2px solid;BORDER-TOP: #b2bfa0 2px solid;">
			<TR><TD><xsl:value-of select="$nbsp" disable-output-escaping="yes"/><b><font color="{$title_color}" style="text-transform: uppercase">
				<!-- start uneditable zone -->
				<xsl:choose>
					<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
					<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
	...
	<xsl:template name="margin_design">
		...
		<table width="100%" cellpadding="0" cellspacing="0">
	        <xsl:if test="string-length(showtitle)=0 or showtitle='on'">
		        <tr> 
		        	<td style="BORDER-BOTTOM: #333333 2px solid;BORDER-LEFT: #b2bfa0 2px solid;BORDER-RIGHT: #333333 2px solid;BORDER-TOP: #b2bfa0 2px solid;" width="100%" bgcolor="{$menuheadleft_background}"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
		          	<font color="{$menuheadleft_color}" style="text-transform: uppercase;"><b>
		          		<!-- start uneditable zone -->
					<xsl:choose>
						<xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when>
						<xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise>
	        ...
	        <tr>
	        <td width="100%" colspan="2" bgcolor="{$menuleft_background}">
	        	<!-- start uneditable zone -->
				<xsl:choose>
	...
	<xsl:template name="rightmargin_design">
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
	        <xsl:if test="string-length(showtitle)=0 or showtitle='on'">
		        <tr> 
		         	<td style="BORDER-BOTTOM: #333333 2px solid;BORDER-LEFT: #b2bfa0 2px solid;BORDER-RIGHT: #333333 2px solid;BORDER-TOP: #b2bfa0 2px solid;" bgcolor="{$menuheadright_background}" width="100%"><xsl:value-of select="$nbsp" disable-output-escaping="yes"/> 
		          		<font color="{$menuheadright_color}" style="text-transform: uppercase;"><b><xsl:choose><xsl:when test="contains($title, '&lt;')"><xsl:value-of select="$title" disable-output-escaping="yes"/></xsl:when><xsl:otherwise><xsl:copy-of select="$title" /></xsl:otherwise></xsl:choose></b></font>
		          	</td>
		        </tr>
	        </xsl:if>
	...
	<xsl:template name="toolbar_global_design">
		<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0" style="border-top: 1px solid {$toolbar_border}; border-bottom: 1px solid {$toolbar_border};">
			<TR bgColor="#6a7c52" height="25"><xsl:call-template name="toolbar_body_macro"/></TR>
		</TABLE>				
	</xsl:template>
	...
	<xsl:template name="toolbar_lastitem_design">
		<xsl:param name="content"/>		
			<td style="text-transform: uppercase; border-right: 1px solid white;" align="center" width="200"><font color="#EEEEE2"><b>
			<!-- start uneditable zone -->
			<xsl:choose>
				<xsl:when test="contains($content, '&lt;')"><xsl:value-of select="$content" disable-output-escaping="yes"/></xsl:when>
				<xsl:otherwise><xsl:copy-of select="$content" /></xsl:otherwise>
