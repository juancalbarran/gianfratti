...
Function BuscaDataBancoDados(Data)
	...
	SQL	=	"SELECT  CONVERT(CHAR, Data_Cadastro, 103) AS Data_Cadastro " &_
			"FROM dbo.HomeCare_Atendimento " &_ 
			"WHERE (CONVERT(CHAR, Data_Cadastro, 103) = '"&Data&"') And id_Atendimento_Status_Condicoes_Alta = 1 " 
			Set rs = server.createobject("adodb.recordset")
			Rs.open SQL, CONN, 3
				IF not rs.eof then 'Verifica se existem registros
				
					BuscaDataBancoDados = Rs("Data_Cadastro")
